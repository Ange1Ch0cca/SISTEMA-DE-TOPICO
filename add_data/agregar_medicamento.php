...
$sql = "
    SELECT id, nombre, (cantidad_ingreso - IFNULL(salida, 0)) AS stock_final
    FROM (
        SELECT m.id, m.nombre, m.cantidad_ingreso, 
               (SELECT IFNULL(SUM(ai.cantidad), 0) 
                FROM atencion_insumos ai 
                WHERE ai.insumo_medico = m.nombre) AS salida
        FROM medicamentos m
        UNION ALL
        SELECT mm.id, mm.nombre, mm.cantidad_ingreso, 
               (SELECT IFNULL(SUM(ai.cantidad), 0) 
                FROM atencion_insumos ai 
                WHERE ai.insumo_medico = mm.nombre) AS salida
        FROM mmedicamentos mm
    ) AS medicamentos_totales
    HAVING stock_final >= 1"; // Filtrar solo los medicamentos con stock mayor o igual a 1
                                ...
                                <script>
                                    ...
                                    document.getElementById('addInsumoBtn').addEventListener('click', function() {
                                        ...
                                        if (!medicamentoId || !cantidad || cantidad <= 0) {
                                            alert("Por favor, selecciona un medicamento y una cantidad válida.");
                                            return;
                                        }
                                        ...
                                        xhr.onload = function() {
                                            if (xhr.status === 200) {
                                                ...
                                                if (response.success) {
                                                    ...
                                                    alert("Hubo un error al guardar los insumos.");
                                                }
                                            } else {
