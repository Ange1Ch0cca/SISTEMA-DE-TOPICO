...
// Consultar el id de la última atención (usando el campo `id` autoincremental)
$sql = "SELECT id FROM atenciones ORDER BY id DESC LIMIT 1";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // Obtener el id de la última atención
    $row = $result->fetch_assoc();
    $id_atencion = $row['id'];

    // Ahora obtenemos los detalles de la última atención
    $sql_patient_details = "SELECT nombres, apellidos, programa_estudios, fecha_atencion, dni, semestre, procedimiento FROM atenciones WHERE id = ?";
    $stmt = $conn->prepare($sql_patient_details);
    $stmt->bind_param("i", $id_atencion); // Vinculamos el id de la atención
    $stmt->execute();
    $stmt->store_result();

    if ($stmt->num_rows > 0) {
        // Asociamos las columnas a variables
        $stmt->bind_result($nombres, $apellidos, $programa_estudios, $fecha_atencion, $dni, $semestre, $procedimiento);
        $stmt->fetch();
    } else {
        // Si no se encuentra la atención
        $nombres = $apellidos = $programa_estudios = $fecha_atencion = $dni = $semestre = $procedimiento = "No disponible";
    }
} else {
    // Si no hay atenciones registradas
    $id_atencion = $nombres = $apellidos = $programa_estudios = $fecha_atencion = $dni = $semestre = $procedimiento = "No disponible";
}
...
if ($result->num_rows > 0) {
    ...
    // Obtener los insumos médicos relacionados con la última atención
    $sql_insumos = "SELECT ai.codigo, ai.insumo_medico, ai.cantidad 
                    FROM atencion_insumos ai 
                    WHERE ai.atencion_id = ?";
